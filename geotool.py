...
def interpolate(geoloc, time, elv):
    geoloc=np.array(geoloc)
    print('{} \n{}\n{}'.format(geoloc, geoloc.shape, time))
    time= np.array(time)
    elv=np.array(elv)
    #Note: Lat long backwards (need to fix)
    pos_data = [[latlonelv_xyz(geoloc[i,1], geoloc[i,0], elv[i])] for i in range(geoloc.shape[0])]
    ...
    return xyz_latlonelv(p.reshape(-1))
